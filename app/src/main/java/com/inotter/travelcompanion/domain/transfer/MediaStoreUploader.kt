...
class MediaStoreUploader(
     ...
     * @return OutputStream positioned at the end for appending, or null if open failed
     */
    fun getAppendOutputStream(uri: Uri): OutputStream? {
        return try {
            // Open in append mode ("wa" = write append)
     ...
     * Used to determine resume offset for interrupted uploads.
     *
     * @param uri Content URI of the MediaStore entry
     * @return Current file size in bytes, or 0 if query failed
     */
    fun getFileSize(uri: Uri): Long {
        return try {
            ...
            android.util.Log.w("MediaStoreUploader", "Failed to get file size: ${e.message}")
            0L
        }
    }

    /**
     * Checks if a MediaStore entry is still pending (incomplete upload).
     *
     * @param uri Content URI of the MediaStore entry
     * @return true if the entry exists and IS_PENDING=1, false otherwise
     */
    fun isPending(uri: Uri): Boolean {
        ...
        return try {
            ...
            android.util.Log.w("MediaStoreUploader", "Failed to check pending status: ${e.message}")
            false
        }
    }
